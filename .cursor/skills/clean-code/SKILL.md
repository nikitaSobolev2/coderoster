---
name: clean-code
description: Uncle Bob Clean Code plus SOLID, KISS, DRY — naming, functions, structure, and maintainability
---

# Clean Code (Uncle Bob)

Guidance from _Clean Code_, extended with **SOLID**, **KISS**, and **DRY** for structure and change.
...
- **S — Single responsibility**: One reason to change per module, class, or cohesive unit of behavior.
- **O — Open/closed**: Extend behavior without modifying stable core code; favor composition, strategies, and hooks over editing shared internals for every new case.
- **L — Liskov substitution**: Subtypes must honor the contract of their supertypes so callers can substitute them without surprises.
- **I — Interface segregation**: Keep APIs small and focused; do not force clients to depend on operations they do not use.
- **D — Dependency inversion**: Depend on abstractions; high-level policy should not depend on low-level implementation details.

## KISS

- Prefer the simplest design that correctly solves the problem.
- Avoid unnecessary abstraction, deep nesting, and cleverness that hides intent.
- Use straightforward control flow and clear names; reach for patterns when they reduce complexity, not by default.

## DRY

- Each piece of knowledge or rule should have a **single source of truth**; fix behavior in one place.
- Duplication of _text_ is a weak signal; duplication of _business rules_ or _invariants_ is the real cost.
- Do not merge unrelated concepts just to deduplicate—sometimes a small repeat is cheaper than the wrong abstraction.
...
- Align class boundaries with SOLID (especially single responsibility and dependency inversion).
