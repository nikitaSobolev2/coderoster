...
export default function ProfileCard({ initial }: Props) {
  ...
  const isDirty = computeDirty(state, initial)
  ...
  return (
    <form className={styles.form} onSubmit={onSubmit} noValidate>
      ...
      <TextInput
        ...
        required
        maxLength={40}
        description="Используется в адресе профиля и комментариях."
      />
      <Textarea
        label="Био"
        value={state.bio}
      ...
      <div className={styles.form__actions}>
        ...
        {!isDirty ? <span className={styles.form__hint}>Нет изменений</span> : null}
      </div>
    </form>
  )
}
...
function computeDirty(state: FormState, initial: UserSettings): boolean {
  const initialState = toFormState(initial)
  return (
